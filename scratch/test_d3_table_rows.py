...
with open('REC2608010_D3_Table_Rows.pdf', 'wb') as f:
    f.write(res.content)

doc = fitz.open('REC2608010_D3_Table_Rows.pdf')
page1 = doc[0]
print("=== D3 TABLE ROWS AUDIT ===")
for b in page1.get_text('blocks'):
    txt = b[4].strip().replace('\n', ' ')
    if 'Autre action' in txt or 'Commentaires' in txt:
        print(f"Block bbox ({b[0]:.1f}, {b[1]:.1f}, {b[2]:.1f}, {b[3]:.1f}) -> {repr(txt)}")

print("\nD3 TABLE AUDIT SUCCESSFUL!")
